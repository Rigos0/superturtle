...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after adding device-flow poll backoff support for hosted login `slow_down` responses. Most recently, hosted session writes now create exclusive random temp files before atomic rename so pre-created predictable temp-path symlinks cannot redirect login or refresh writes, the existing `lstat`-based session-path and parent-directory validation remains in place to reject symlinked session files and symlinked config directories, and login polling now treats retryable `429` / `slow_down` control-plane responses as backoff signals instead of aborting the browser login flow.
