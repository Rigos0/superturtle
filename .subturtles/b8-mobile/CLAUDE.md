...
- [ ] Mobile-fix messaging in `linkedin-demo/src/components/messaging/Style.js`: At xs breakpoint, set `root` height to `calc(100vh - 110px)` to account for bottom nav. Ensure input bar doesn't get hidden behind bottom nav. Thread header back button needs adequate tap target. <- current
