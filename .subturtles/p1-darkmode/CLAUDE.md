## Current Task
Fix notifications dark mode in `linkedin-demo/src/components/notifications/Style.js`: Replace `backgroundColor: "#fff"` in items with `theme.palette.background.paper`. Replace `#f8fcf8` unread bg with a dark-mode-friendly subtle green tint: `theme.palette.type === "dark" ? "rgba(46,125,50,0.08)" : "#f8fcf8"`. Replace `#f4f8f4` hover with `theme.palette.action.hover`. Fix `#1d2226` text to `theme.palette.text.primary`. Fix borders `#eceff1`/`#f1f3f4` to `theme.palette.divider`.
...
- [ ] Fix notifications dark mode in `linkedin-demo/src/components/notifications/Style.js`: Replace `backgroundColor: "#fff"` in items with `theme.palette.background.paper`. Replace `#f8fcf8` unread bg with a dark-mode-friendly subtle green tint: `theme.palette.type === "dark" ? "rgba(46,125,50,0.08)" : "#f8fcf8"`. Replace `#f4f8f4` hover with `theme.palette.action.hover`. Fix `#1d2226` text to `theme.palette.text.primary`. Fix borders `#eceff1`/`#f1f3f4` to `theme.palette.divider`. <- current
- [ ] Fix network cards dark mode in `linkedin-demo/src/components/network/Style.js`: Replace card `backgroundColor: "#fff"` with `theme.palette.background.paper`. Replace search field `backgroundColor: "#fff"` with `theme.palette.background.paper`.
- [ ] Fix post edit textarea dark mode in `linkedin-demo/src/components/posts/post/Style.js`: In `editTextarea` class, add dark mode: `backgroundColor: theme.palette.background.paper`, `color: theme.palette.text.primary`, `border: "1px solid " + theme.palette.divider`. In `cancelButton`, adapt border color. In `comment__form`, check input colors (line 261-262 already has dark mode — verify it works).
