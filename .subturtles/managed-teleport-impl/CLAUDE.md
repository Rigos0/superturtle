...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after adding on-read permission hardening so valid local hosted session files are re-secured to `0600` without touching unsupported future-schema files.
