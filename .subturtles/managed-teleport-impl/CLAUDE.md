...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after hardening local hosted session file handling to require a regular file on disk. Most recently, hosted session writes now create exclusive random temp files before atomic rename so pre-created predictable temp-path symlinks cannot redirect login or refresh writes, and the existing `lstat`-based session-path and parent-directory validation remains in place to reject symlinked session files and symlinked config directories.
