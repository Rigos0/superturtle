...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after extending cached fallback behavior to cover transient control-plane HTTP outages in addition to socket-level failures.
