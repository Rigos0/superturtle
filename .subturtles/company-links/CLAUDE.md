...
- [ ] Add company follow notification: in `src/convex/companyFollowers.ts` followCompany mutation, after inserting the follow, create a notification for each company admin: type "company_follow", fromUserId = follower, include companyId. Add companyId as optional field to notifications schema if needed. <- current
