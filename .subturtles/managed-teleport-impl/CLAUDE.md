...
Continue the hosted auth foundation by tightening durable hosted session semantics for `superturtle login` / `whoami` / `cloud status`, with a focus on the remaining hosted login/session contract work after extending fail-closed control-plane payload validation to hosted identity and cloud-status reads as well as login start, login completion, and session refresh responses.
