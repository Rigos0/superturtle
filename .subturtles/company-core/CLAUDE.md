## Current Task
Add `companyFollowers` table to schema.ts with company/user indexes.
...
- [ ] Add `companyFollowers` table to schema.ts: companyId (companies id), userId (users id), createdAt (number). Index on companyId and byCompanyAndUser. <- current
